...
namespace Radios
{
    ...
    public class FlexBase : IDisposable
    {
        ...
        private const string noStation = "No station was setup.";
        ...
        public void Disconnect()
        {
            ...
            try
            {
                if ((mainThread != null) && mainThread.IsAlive)
                {
                    ...
                    stopMainThread = true;
                    if (mainThread.Join(30000))
                    {
                        Tracing.TraceLine("Disconnect:main thread stopped", TraceLevel.Info);
                    }
                    else Tracing.TraceLine("Disconnect:main thread didn't stop", TraceLevel.Error);
                }
            }
            catch (Exception ex)
            {
        ...
        private void radioPropertyChangedHandler(object sender, PropertyChangedEventArgs e)
        {
            ...
            {
                Tracing.TraceLine("dbgRadioNotMine:" + e.PropertyName);
            }
            switch (e.PropertyName)
            {
                ...
                case "ATUTuneStatus":
                    {
                        ...
                        switch (theRadio.ATUTuneStatus)
                        {
                            ...
                            case ATUTuneStatus.Bypass:
                                ...
                                if (FlexTunerOn) FlexTunerOn = false;
                                // Turn off autoTune.
                                setFlexTunerTypeNotAuto();
                                break;
                ...
                case "TX1Enabled":
                    ...
                    // keep it enabled.
                    if (!r.TX1Enabled) q.Enqueue((FunctionDel)(() => { theRadio.TX1Enabled = true; }), "TX1Enabled");
                    break;
                    ...
                    case "DemodMode":
                        {
                            ...
                            if (s.Active) FilterObj.RXFreqChange(s);
#if CWMonitor
                            try
                            {
        ...
        private void panadapterPropertyChangedHandler(object sender, PropertyChangedEventArgs e)
        {
            ...
            {
                Tracing.TraceLine("dbgPanNotMine:" + e.PropertyName);
            }
        ...
        private void waterfallPropertyChangedHandler(object sender, PropertyChangedEventArgs e)
        {
            ...
            {
                Tracing.TraceLine("dbgFallNotMine:" + e.PropertyName);
            }
        ...
        private void guiClientPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            if (client == null)
            {
                Tracing.TraceLine("guiClientPropertyChanged:client is null", TraceLevel.Error);
                return;
            }
...
#region Rig properties
        ...
        public int RXVFO
        {
            ...
            set
            {
                if (_RXVFO != value)
                {
                    ...
                    if (ValidVFO(_RXVFO))
                    {
                        q.Enqueue((FunctionDel)(() => { VFOToSlice(_RXVFO).Active = true; }), "Active");
                    }
        ...
        public int TXVFO
        {
            ...
            set
            {
                if (_TXVFO != value)
                {
                    ...
                    if (ValidVFO(_TXVFO))
                    {
                        q.Enqueue((FunctionDel)(() => { mySlices[_TXVFO].IsTransmitSlice = true; }), "IsTransmitSlice");
                    }
                }
            }
        }
        ...
        public string RXMode
        {
            ...
            set
            {
                q.Enqueue((FunctionDel)(() => { theRadio.ActiveSlice.DemodMode = value; }), "RXDemodMode");
            }
        }
        ...
        public FlexTunerTypes FlexTunerType
        {
            ...
            set
            {
                ...
                Tracing.TraceLine("FlexTunerType:" + value.ToString() + ' ' +
                    _FlexTunerType.ToString() + ' ' + FlexTunerOn.ToString(), TraceLevel.Info);
        ...
        private bool _FlexTunerOn; // always set by the TXTune property.
        public bool FlexTunerOn
        {
            ...
            set
            {
                ...
                Tracing.TraceLine("FlexTunerOn:" + value.ToString() + ' ' +
                    FlexTunerOn.ToString() + ' ' + _FlexTunerType.ToString(), TraceLevel.Info);
                if (value == FlexTunerOn) return;
                switch (_FlexTunerType)
                {
                    case FlexTunerTypes.manual:
                        {
                            Callouts.SWRThreshold = 1.4f; // temp
                            Callouts.ManualTunerTimeout = new TimeSpan(5000 * 10000); // temp - 5 sec
                            float highSWR = 0f;
                            if (!value)
                            {
                                // turned off manually
                            ...
                            q.Enqueue((FunctionDel)(() =>
                            {
                                theRadio.TXTune = value;
                                DbgTrace("dbgTune value:" + theRadio.TXTune.ToString());
                                // If starting, and have a threshold,
                                if (value & (Callouts.SWRThreshold != 0f))
                                {
                                    DateTime tunerStartTime = DateTime.Now;
                                    TimeSpan delta;
                                    // await tuner startup.
                                    Thread.Sleep(1000);
                                    // Look for minimum SWR,
                                    // while we need to keep tuning and we're making progress.
                                    DbgTrace("dbgTune looking");
                                    do
                                    {
                                        highSWR = _SWR;
                                        Thread.Sleep(10);
                                        delta = DateTime.Now - tunerStartTime;
                                    } while (
                                        ((_SWR > Callouts.SWRThreshold) &
                                        (highSWR >= _SWR) &
                                        (delta < Callouts.ManualTunerTimeout))
                                        );
                                    DbgTrace("dbgTune SWR values:" + Callouts.SWRThreshold.ToString() + ' ' + _SWR.ToString() + ' ' + highSWR.ToString());
                                    Thread.Sleep(100); // .1 sec delay
                                    highSWR = _SWR;
                                    theRadio.TXTune = false;
                                    DbgTrace("dbgTune value:" + theRadio.TXTune.ToString());
                                    // Report status.
                                    ATUTuneStatus stat = ATUTuneStatus.OK;
                                    RaiseFlexAntTuneStartStop(new FlexAntTunerArg
        ...
        public int AudioGain
        {
            ...
            set
            {
                q.Enqueue((FunctionDel)(() => { theRadio.ActiveSlice.AudioGain = value; }), "AudioGain");
            }
        }
        ...
        public int AudioPan
        {
            ...
            set
            {
                q.Enqueue((FunctionDel)(() => { theRadio.ActiveSlice.AudioPan = value; }), "AudioPan");
            }
        }
        ...
        internal int NoiseBlankerLevel
        {
            ...
            set { q.Enqueue((FunctionDel)(() => { theRadio.ActiveSlice.NBLevel = value; }), "NBLevel"); }
        }
        ...
        internal int WidebandNoiseBlankerLevel
        {
            ...
            set { q.Enqueue((FunctionDel)(() => { theRadio.ActiveSlice.WNBLevel = value; }), "WNBLevel"); }
        }
        ...
        internal AGCMode AGCSpeed
        {
            ...
            set { q.Enqueue((FunctionDel)(() => { theRadio.ActiveSlice.AGCMode = value; }), "AGCMode"); }
        }
        ...
        public virtual void TestRoutine()
        {
            ...
            remoteAudioThread.Priority = (remoteAudioThread.Priority == ThreadPriority.Normal) ?
                ThreadPriority.Highest : ThreadPriority.Normal;
            Tracing.TraceLine("TestRoutine:" + remoteAudioThread.Priority.ToString(), TraceLevel.Info);
            //DialogResult rv = MessageBox.Show("disconnect?", "query", MessageBoxButtons.YesNo);
            //if (rv == DialogResult.Yes) theRadio.Disconnect();
        }
#endregion
                    ...
                    lock (stream.OpusRXListLockObj)
                    {
                        ...
                        else
                        {
                            ...
                            if ((lastID != -1) &&
                                (stream.LastOpusTimestampConsumed != stream._opusRXList.Keys[lastID]))
                            {
                                if (lastID < priorLastID)
                                {
                                    // overflow
                                    priorLastID = -1;
                                }
                                for (priorLastID += 1; priorLastID <= lastID; priorLastID++)
                                {
                                    payloadList.Add(stream._opusRXList.Values[priorLastID].payload);
                                }
                                stream.LastOpusTimestampConsumed = stream._opusRXList.Keys[lastID];
                            }
                            // else ((lastID == -1) || (priorLastID already == lastID))
                            else priorLastID = lastID;
                        }
                    }
                    // get this out from under the opus save lock.
                    foreach (byte[] p in payloadList)
                    {
                        opusOutputChannel.PortAudioStream.WriteOpus(p);
#if opusToFile
                                        writeOpus(p);
#endif
                    }
...
#region parms
        ...
        public class OpenParms
        {
            ...
            /// <summary>
            /// SWR threshold for stopping the manual tuner.
            /// </summary>
            public float SWRThreshold;
            /// <summary>
            /// timeout for the manual tuner auto stop.
            /// </summary>
            public TimeSpan ManualTunerTimeout;
            /// <summary>
...
#region mainThread
        ...
        private void mainThreadProc()
        {
            ...
            try
            {
                // must be able to get at least 1 slice.
                if (initialFreeSlices==0)
                {
                    Tracing.TraceLine("Flex open:No free slices", TraceLevel.Error);
                ...
                {
                    Tracing.TraceLine("Flex open:didn't get a station name:should be " + Callouts.StationName, TraceLevel.Error);
                ...
                while (!stopMainThread)
                {
                    while (q.Count > 0)
                    {
                        ...
                        DbgTrace("mainLoop:done " + el.Name);
                    }
