...
namespace Radios
{
    ...
    public class FlexBase : IDisposable
    {
        ...
        public bool Connect(string serial, bool lowBW)
        {
            ...
            raiseConnectedEvent(theRadio.Connected);
            return rv;
        }
                    ...
                    case "DemodMode":
                        {
                            ...
                            if (s.Active)
                            {
                                Tracing.TraceLine("dbg DemodMode");
                                FilterObj.RXFreqChange(s);
                            }
#if CWMonitor
                            try
        ...
        internal int SidetonePitch
        {
            ...
            set
            {
                ...
                q.Enqueue((FunctionDel)(() => { theRadio.CWPitch = value; }));
            }
        }
        ...
        public bool SaveProfile(Profile_t p)
        {
...
#region mainThread
        ...
        // Used when attempting reconnect.
        private const int reconInitial = 100;
        private const int reconFinal = 1000;
        ...
        private void mainThreadProc()
        {
            ...
            try
            {
                ...
                {
                    Tracing.TraceLine("Flex open:couldn't get a slice", TraceLevel.Error);
                ...
                int reconSleep = reconInitial;

                // Main loop.
                while (!stopMainThread)
                {
                    if (!Disconnecting & !theRadio.Connected)
                    {
                        Tracing.TraceLine("mainThreadProc:reconnecting", TraceLevel.Info);
                        reconnecting = true;
                        while (reconSleep < reconFinal)
                        {
                            Thread.Sleep(reconSleep);
                            if (theRadio.Connect(clientID))
                            {
                                Tracing.TraceLine("mainThreadProc:reconnected", TraceLevel.Info);
                                reconSleep = reconInitial;
                                reconnecting = false;
                                break;
                            }
                            reconSleep *= 2;
                        ...
                        if (!theRadio.Connected)
                        {
                            Tracing.TraceLine("mainThreadProc:didn't reconnect", TraceLevel.Error);
                            reconnecting = false;
                            stopMainThread = true;
                            continue;
                        }
            ...
            else
            {
                ...
                if (File.Exists(fileName))
                {
                    try
                    {
                        ...
                        BreakinDelay = cfgData.BreakinDelay;
                        SidetoneGain = cfgData.SidetoneGain;
                        SidetonePitch = cfgData.SidetonePitch;
                        CWReverse = cfgData.CWReverse;
                        CWL = (cfgData.CWLEnabled) ?
                            OffOnValues.on : OffOnValues.off;
                        Keyer = cfgData.Keyer;
                        KeyerSpeed = cfgData.KeyerSpeed;
                    }
                    catch (Exception ex)
                    {
        ...
        internal bool GetProfileInfo(bool postImport)
        {
            ...
            if (postImport)
            {
                ...
                if (theRadio.ActiveSlice != null)
                {
                    Tracing.TraceLine("dbg PostImport");
                    FilterObj.RXFreqChange(theRadio.ActiveSlice);
                }
                raisePowerEvent(true);
