...
namespace Flex.Smoothlake.FlexLib
{
    ...
    public class Radio : ObservableObject
    {
        ...
        private Thread _statisticsThread = null;
        ...
        public bool Connect()
        {
            ...
            SendCommand("client set enforce_network_mtu=1 network_mtu=1500");
        ...
        private void StartKeepAlive()
        {
            ...
            if (_keepAliveThread == null)
            {
                ...
                _keepAliveThread.Name = "Keep Alive Thread";
                _keepAliveThread.IsBackground = true;
                _keepAliveThread.Priority = ThreadPriority.Normal;
                _keepAliveThread.Start();
            }
        }

        private HiPerfTimer _keepAliveTimer = new HiPerfTimer();
        private HiPerfTimer _rttTimer = new HiPerfTimer();
        /* This will just be a free running timer used to report a ms timing to 
         * the radio for jitter measurements. */
        private HiPerfTimer _jitterTimer = new HiPerfTimer();
        private double _outstandingRTTStart = 0;
        ...
        private void KeepAlive()
        {
            ...
            while (_connected)
            {
                ...
                _jitterTimer.Stop();
                _rttTimer.Stop();
                if (_outstandingRTTStart == 0)
                    _outstandingRTTStart = _rttTimer.DurationMsec;
                if (!program_name.Contains("Maestro"))
                {
                    SendReplyCommand(new ReplyHandler(GetPingReply), "ping ms_timestamp=" + _jitterTimer.DurationMsec);
                }
                else
                {
        ...
        private void GetPingReply(int seq, uint resp_val, string reply)
        {
            ...
            if ( _outstandingRTTStart != 0 )
                _lastPingRTT = _rttTimer.DurationMsec - _outstandingRTTStart;

            _outstandingRTTStart = 0;
        }
        ...
        private void Statistics_ThreadFunction()
        {
            while (_connected)
            {
        ...
        private void ParseStatus(string s)
        {
            ...
            switch (words[0])
            {
                ...
                case "audio_stream":
                    {
                        if (words.Length < 4)
                        {
                            Debug.WriteLine("ParseStatus: Too few words for audio_stream status -- min 4 (" + s + ")");
                            return;
                        }
                        ...
                        bool add_new = false;
                        // do we have an existing object associated with this stream ID?
                        AudioStream audio_stream = FindAudioStreamByStreamID(stream_id);
                        if (audio_stream == null)
                        {
                            // no -- if we are killing the audio stream, don't worry about it
                            if (s.Contains("in_use=0")) return;

                            // is this stream for this client?
                ...
                case "client":
                    //client <handle> disconnected forced=<0/1>
                    if (words.Length < 4)
                    {
                    ...
                    string handle = words[1];
                    string command = words[2];
                    string forced = words[3];

                    // strip off "0x" from handle
                    if (handle.StartsWith("0x") || handle.StartsWith("0X"))
                        handle = handle.Substring("0x".Length);

                    if (handle == _client_id && command == "disconnected" && forced == "forced=1")
                    {
        ...
        {
            string cmd = "spot add callsign=" + spot.Callsign + " rx_freq=" + StringHelper.DoubleToString(spot.RXFrequency, "f6");
        ...
        internal void OnSpotTriggered(Spot spot)
        {
        ...
        private void ParseUsbCableStatus(string s)
        {
            ...
            if (cable == null)
            {
                ...
                else if (s.Contains("type=bcd_vbcd"))
        ...
        public void AddCableOfType(string value, string serialNumber)
        {
            switch (value)
            {
                case "cat":
                    AddUsbCable(new UsbCatCable(this, serialNumber));
                    break;
                case "bcd":
                    AddUsbCable(new UsbBcdCable(this, serialNumber, "bcd"));
                    break;
                case "vbcd":
                    AddUsbCable(new UsbBcdCable(this, serialNumber, "vbcd"));
                    break;
                case "bcd_vbcd":
                    AddUsbCable(new UsbBcdCable(this, serialNumber, "bcd_vbcd"));
                    break;
                case "bit":
                    AddUsbCable(new UsbBitCable(this, serialNumber));
                    break;
                case "invalid":
                    AddUsbCable(new UsbOtherCable(this, serialNumber, UsbCableType.Invalid));
                    break;
            }
        }
        ...
        public void RequestAudioStream(int channel)
        {
            SendCommand("stream create dax=" + channel);
        }

        public void RequestMICAudioStream()
        {
            SendCommand("stream create daxmic");
        }

        public void RequestTXAudioStream()
        {
            SendCommand("stream create daxtx");
        }

        public void RequestIQStream(int channel)
        {
            SendCommand("stream create daxiq=" + channel);
        }
        ...
        public Panadapter FindPanByDAXIQChannel(int daxIQChannel)
        {
            lock (_panadapters)
            {
                Panadapter pan = _panadapters.FirstOrDefault(x => x.DAXIQChannel == daxIQChannel);
                return pan;
            }
        }
        ...
        private bool IsAudioStreamStatusForThisClient(string s)
        {

            string status_client_handle_str = null;
            ...
            for (int i=2; i<words.Length; i++)
            {
                ...
                if (tokens.Length != 2)
                {
                    Debug.WriteLine("Radio::IsAudioStreamStatusForThisClient: Invalid key/value pair (" + words[i] + ")");
                    continue;
                }
                ...
                switch (key.ToLower())
                {
                    case "client_handle": status_client_handle_str = value; break;
                }
            }

            if (status_client_handle_str == null || status_client_handle_str == "" )
                return false;

            if ( _client_id == status_client_handle_str.Substring("0x".Length)) // Strip 0x from client_handle
                return true;
        ...
        {
            if (_opusStreams == null) return null;
            
            lock (_opusStreams)
            {
                if (_opusStreams.Count > 0 && _opusStreams[0] != null)
                {
                    ...
                    case "num":
                        {
                            ...
                            // strip the 0x on hex numbers if needed
                            if (value.StartsWith("0x"))
                            {
                                value = value.Substring(2);
                                b = int.TryParse(value, NumberStyles.HexNumber, null, out source_index);
                            }
                            else
                            {
        ...
        public InterlockState InterlockState
        {
            ...
            internal set
            {
                if (_interlockState != value)
                {
                    ...
                    else if(_interlockState != InterlockState.PTTRequested && _interlockState != InterlockState.UnkeyRequested) // don't undo MOX if we are waiting on TX Req or Unkey Req
                    {
                        if (_mox != false)
                        {
                            _mox = false;
                    ...
                    case "reason":
                        {
                            ...
                            if (reason == InterlockReason.None && !value.Contains("PG-XL"))
                            {
        ...
        private void Private_MonitorNetworkQuality(object obj)
        {
            ...
            while (_connected)
            {
                ...
                if (_opusStreams.Count > 0)
                {
                    ...
                    totalPacketCount += _opusStreams[0].OpusPacketTotalCount;
                }
        ...
        public void CWPTT(bool state, string timestamp)
        {
            if (_netCWStream != null)
            {
                string cmd = "cw ptt " + Convert.ToByte(state) + " time=0x" + timestamp + " index=" + _netCWStream.GetNextIndex();
                // SendCommand( "cw ptt " + Convert.ToByte(state) + " time=0x" + timestamp
       ...
       private void UpdateProfileList(string profile_type, string list)
       {
        ...
        {
            _statisticsThread = new Thread(new ThreadStart(Statistics_ThreadFunction));
            _statisticsThread.Name = "Statistics Thread";
            _statisticsThread.Priority = ThreadPriority.Normal;
            _statisticsThread.IsBackground = true;
       ...
       private void Private_SendMetaSubsetFile(object obj)
       {
        ...
        public string RadioLicenseId
        {
            ...
            internal set
            {
                if (_radioLicenseId != value)
                {
                    ...
                    RaisePropertyChanged("RadioLicenseId");
                }
            }
        }
        ...
        {
            get { return _requiresAdditionalLicense; }
            internal set
            {
                if (_requiresAdditionalLicense != value)
                {
                    _requiresAdditionalLicense = value;
                    RaisePropertyChanged("RequiresAdditionalLicense");
                }
            }
        }
        ...
        {
            get { return _frontPanelMacAddress; }
            internal set
            {
                if (_frontPanelMacAddress != value)
                {
                    _frontPanelMacAddress = value;
                    RaisePropertyChanged("FrontPanelMacAddress");
                }
            }
        }
        ...
        private void RegisterUDP()
        {
            try
            {
                while (VitaSock != null && !_udpSuccessfulRegistration && Connected)
                {
                    ...
                    Byte[] sendBytes = Encoding.ASCII.GetBytes("client udp_register handle=0x" + ClientID);
                ...
                while ( VitaSock != null && Connected )
                {
                     ...
                     */
                    Byte[] sendBytes = Encoding.ASCII.GetBytes("client ping handle=0x" + ClientID);
                    VitaSock.SendUDP(sendBytes);
                    Thread.Sleep(5000);
                }
            }
        ...
        private void StartUDP( )
        {
            if (IsWan)
            {
                if (RequiresHolePunch)
                {
                    VitaSock = new VitaSocket(NegotiatedHolePunchPort, UDPDataReceivedCallback, _ip, NegotiatedHolePunchPort, ClientID);
                }
                else
                {
                    VitaSock = new VitaSocket(4991, UDPDataReceivedCallback, IP, PublicUdpPort, ClientID);
                }
            ...
            {
                VitaSock = new VitaSocket(4991, UDPDataReceivedCallback, IP, 4991, ClientID);
            }
