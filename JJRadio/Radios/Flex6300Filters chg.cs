...
namespace Radios
{
    public partial class Flex6300Filters : UserControl
    {
        ...
        public Flex6300Filters(Flex6300 r, AllRadios.OpenParms openParms)
        {
            ...
            buttonControls.Add(FlexKnobButton);
            ...
            // Setup the control knob if any.
            knob = new FlexKnob(rig);
        }
        ...
        private void updateBoxes()
        {
            ...
            try
            {
                ...
                // Set the control knob step size.
                //knob.SetKnobStepSizeChangePerMode(rig.Mode.ToString());
        ...
        private void FlexKnobButton_Click(object sender, EventArgs e)
        {
            knob.Config();
        }
        ...
        private void panParameterSetup()
        {
            ...
            try
            {
                ...
                Tracing.TraceLine("dbg1:" + ((brailleWidth * brailleScaleup) + brailleWidth).ToString());
                ...
                Tracing.TraceLine("dbg2:" + fps.ToString());
                rig.q.Enqueue((Flex.FunctionDel)(() => { panadapter.FPS = fps; }));
                Tracing.TraceLine("dbg3:" + (rig.LongFreqToLibFreq(segment.Low + (ulong)(width / 2))).ToString());
                rig.q.Enqueue((Flex.FunctionDel)(() => { panadapter.CenterFreq = rig.LongFreqToLibFreq(segment.Low + (ulong)(width / 2)); }));
                Tracing.TraceLine("dbg4:" + (rig.LongFreqToLibFreq(segment.Low + (ulong)(width / 2))).ToString());
        ...
        private void PanBox_Enter(object sender, EventArgs e)
        {
            ...
            lock (currentPanData)
            {
                ...
                Tracing.TraceLine("dbg20:" + pos.ToString());
                currentPanData.EntryPosition = pos;
            }
            Thread panEnterThread = new Thread(panEnterThreadProc);
            panEnterThread.Name = "panEnterThread";
            Tracing.TraceLine("dbg24:" + pos);
            panEnterThread.Start(pos);
        }
        private void panEnterThreadProc(object o)
        {
            int pos = (int)o;
            TextOut.PerformGenericFunction(PanBox, () => { PanBox.SelectionStart = pos; });
            TextOut.PerformGenericFunction(PanBox, () => { PanBox.SelectionLength = 1; });
            Tracing.TraceLine("dbg23:" + pos);
        ...
        private void panTimerHandler(object state)
        {
            ...
            lock (currentPanData)
            {
                ...
                Tracing.TraceLine("dbg21:" + go.ToString() + ' ' + pos.ToString() + ' ' + currentPanData.EntryPosition.ToString());
                if (go)
                {
                    currentPanData.EntryPosition = pos;
                    freq = currentPanData.frequencies[pos];
