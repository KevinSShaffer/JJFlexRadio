...
namespace Radios
{
    ...
    public class Flex : AllRadios
    {
        ...
        {
            Tracing.TraceLine("propertyChanged:" + e.PropertyName, TraceLevel.Verbose);
...
#region mainThread
        ...
        private void mainThreadProc(object o)
        {
            try
            {
                ...
                // Get 2 pan adapters.
                while (!await(() => { return (panCount == 2); }, 2000))
                {
                    theRadio.RequestPanafall();
                }
                if (await(() => { return (theRadio.SliceList.Count == 2); }, 5000))
                {
                    // We have a pan adapter and 2 slices, so we're done.
                    Tracing.TraceLine("flex open radio have pan and slices", TraceLevel.Info);
                    VFOToSlice(RigCaps.VFOs.VFOA).Active = true;
                    // Wait until slice for VFO A is active, and vfo B's isn't.
                    await(() => { return (VFOToSlice(RigCaps.VFOs.VFOA).Active && !VFOToSlice(RigCaps.VFOs.VFOB).Active); }, 1000);
                    ...
                    VFOToSlice(RigCaps.VFOs.VFOB).Mute = true;
        ...
        internal bool GetProfileInfo(bool postImport)
        {
            ...
            Tracing.TraceLine("getProfileInfo:awaiting pan and slice counts == 2", TraceLevel.Info);
            if (!await(() => { return ((panCount >= 2) && (sliceCount >= 2)); }, 1000))
            {
                // Set JJRadioDefault as the current profile.
                theRadio.ProfileGlobalSelection = JJRadioDefault;
                Tracing.TraceLine("getProfileInfo:awaiting JJRadioDefaultSelected and pan and slice counts == 2", TraceLevel.Info);
                if ((rv = await(() =>
                {
                    return (JJRadioDefaultSelected &&
                            ((panCount >= 2) && (sliceCount >= 2)));
                }, 3000)))
                {
                    Tracing.TraceLine("getProfileInfo:got info from profile", TraceLevel.Info);
                }
                else
                {
                    Tracing.TraceLine("getProfileInfo:didn't get info from profile", TraceLevel.Error);
                }
            }
            else
            {
                Tracing.TraceLine("getProfileInfo:had slices and pans", TraceLevel.Info);
                theRadio.ProfileGlobalSelection = JJRadioDefault;
                Tracing.TraceLine("getProfileInfo:awaiting pan and slice counts == 0", TraceLevel.Info);
                if (await(() => { return ((panCount == 0) && (sliceCount == 0)); }, 3000, 5))
                {
                    Tracing.TraceLine("GetProfileInfo:counts now 0", TraceLevel.Info);
                    Tracing.TraceLine("getProfileInfo:awaiting pan and slice counts == 2", TraceLevel.Info);
                    if (await(() => { return (JJRadioDefaultSelected && (panCount >= 2) && (sliceCount >= 2)); }, 3000))
                    {
                        Tracing.TraceLine("getProfileInfo:counts now " + panCount, TraceLevel.Info);
                    }
                    else
                    {
                        Tracing.TraceLine("getProfileInfo:counts not correct " + panCount, TraceLevel.Error);
                    }
                }
                else
                {
                    Tracing.TraceLine("getProfileInfo:counts didn't zero " + panCount, TraceLevel.Error);
                }
            }
