...
Friend Class LogEntry
    ...
    Private Function OpenLog(pos As Long) As Boolean
        ...
        If session Is Nothing Then
            ...
            If rv Then
                Form1.StatusBox.Write(Form1.statLogID, _
            ...
            End If
        Else ' the session is in progress.
        End If
        If Not rv Then
            session = Nothing
        End If
    ...
    Private Sub LogEntry_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Else
            ...
            If Not OpenLog(FilePosition) Then
    ...
    Private Sub setFreqsIfNotSet()
        ...
        If getFieldText(AdifTags.ADIF_RXFreq, dummy) AndAlso (dummy = vbNullString) Then
            ' set from radio.
            ' So not to break logging, use RXFrequency if logging freq not set.
            If RigControl.RXFrequency4Logging = 0 Then
                setFieldText(AdifTags.ADIF_RXFreq, FormatFreq(RigControl.RXFrequency))
            Else
                setFieldText(AdifTags.ADIF_RXFreq, FormatFreq(RigControl.RXFrequency4Logging))
            End If
        End If
