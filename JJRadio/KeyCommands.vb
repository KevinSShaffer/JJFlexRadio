...
Public Class KeyCommands
    ...
    Friend KeyTable() As keyTbl = {
        ...
        New keyTbl(CommandValues.EnterCommand, AddressOf CommandLine,
            "Enter a radio's control command", "radio command", FunctionGroups.general),
        ...
        New keyTbl(CommandValues.DoPanning, AddressOf startPanning,
            "Start/resume panning", "panning", FunctionGroups.general),
        ...
        New keyTbl(CommandValues.DisplayRaw, AddressOf toggleRaw,
            "Toggle rig raw data output.", "raw rig output", FunctionGroups.general),
        ...
        New keyTbl(CommandValues.ReackquireRig, KeyTypes.Command, AddressOf reackquireRtn,
            "Reackquire the rig's values.", "reacquire rig's values", True, FunctionGroups.general),
        ...
        New keyTbl(CommandValues.RemoteAudio, KeyTypes.Command, AddressOf remoteAudioRtn,
            "Remote audio on/off", AddressOf remoteMenuString, False, FunctionGroups.audio),
    ...
    {New KeyDefType(Keys.F1, CommandValues.ShowHelp),
     ...
     New KeyDefType(Keys.F2 Or Keys.Control, CommandValues.EnterCommand),
     ...
     New KeyDefType(Keys.U Or Keys.Control, CommandValues.ShowMenus),
     New KeyDefType(Keys.F2 Or Keys.Control Or Keys.Shift, CommandValues.DisplayRaw),
     ...
     New KeyDefType(Keys.S Or Keys.Control Or Keys.Shift, CommandValues.ReackquireRig),
...
#End If
    ...
    Friend Sub UpdateCWText()
        ...
        For i As Integer = 0 To CWText.Length - 1
            ...
            item = New keyTbl(CWMessageDefs(i).id, KeyTypes.CWText,
                AddressOf sendCWMessage, CWText(i).Label,
                KeyCommands.FunctionGroups.cwMessage)
    ...
    Private Sub DisplayMemory()
        If RigControl IsNot Nothing Then
            Try
                RigControl.RigFields.Memories.ShowDialog()
            Catch ex As Exception
...
#End If
    ...
    Private Sub CommandLine()
        If (RigControl IsNot Nothing) AndAlso NotOnFlex() Then
            Return
        End If
        Dim cmdThread As New Thread(AddressOf cmdProc)
        If CMDLine.ShowDialog() = DialogResult.OK Then
            Form1.ReceivedTextBox.Text = ""
            cmdThread.Start(CMDLine.Buffer)
            Thread.Sleep(0)
            Form1.ReceivedTextBox.Focus()
        End If
    End Sub
    Private Sub cmdProc(o As Object)
        Dim cmd As String = o
        Tracing.TraceLine("cmdproc:" & Escapes.Escapes.Decode(cmd), TraceLevel.Info)
        If OpenParms.RawIO Then
            Dim bytes(cmd.Length - 1) As Byte
            For i As Integer = 0 To cmd.Length - 1
                bytes(i) = AscW(cmd(i))
            Next
            RadioPort.sendBytes(bytes)
            ' Give it a second.
            Thread.Sleep(1000)
        Else
            Dim wasRaw = RigControl.ReportRigOutput
            RigControl.ReportRigOutput(Radios.AllRadios.CommandReporting.inputBased, cmd)
            RadioPort.send(cmd)
            ' Give it a second.
            Thread.Sleep(1000)
            RigControl.ReportRigOutput(wasRaw, "")
        End If
    End Sub

    Shared SyncCmdLock, DataLock As Mutex
    Shared DataReceived As Boolean ' Set when data received
    Shared DataBuffer As String
    Shared fromSyncCmd As Boolean
#If 0 Then
    Friend Function SynchronousCommand(cmd As String) As String
        Tracing.TraceLine("SynchronousCommand:" & cmd, TraceLevel.Info)
        Dim rv As String
        ' One at a time!
        SyncCmdLock.WaitOne()
        fromSyncCmd = True ' for handleDirect()
        ' Command must be formatted correctly.
        ' indicate no data yet.
        DataReceived = False
        DataBuffer = ""
        ' Send the command
        RadioPort.send(cmd)
        ' HandleDirect buffers the output and sets DataReceived.
        While True
            ' give time to post output.
            Thread.Sleep(200)
            DataLock.WaitOne()
            Dim f As Boolean = DataReceived
            DataReceived = False
            DataLock.ReleaseMutex()
            If Not f Then
                Exit While ' no output posted or we're finished.
            End If
        End While
        rv = DataBuffer
        fromSyncCmd = False
        SyncCmdLock.ReleaseMutex()
        Return rv
    End Function
#End If
    Friend Sub HandleDirect(o As Object)
        Dim str As String = Escapes.Escapes.Decode(o.ToString)
        If fromSyncCmd Then
            DataLock.WaitOne()
            DataBuffer &= str
            DataReceived = True
            DataLock.ReleaseMutex()
        Else
            WriteTextX(WindowIDs.ReceiveDataOut, str, 0, False)
        End If
    End Sub
    ...
    Private Sub BeginScan()
        ' A linear scan.
        If (RigControl IsNot Nothing) AndAlso RigControl.IsOpen AndAlso RigControl.Power AndAlso _
    ...
    Private Sub memScan()
        If (RigControl IsNot Nothing) AndAlso RigControl.IsOpen AndAlso RigControl.Power AndAlso _
           (Not RigControl.Transmit) AndAlso (scanstate <> scans.linear) Then
            ' Memory scan
...
#End If
    ...
    ''' <summary>
    ''' Change command reporting to either none or raw.
    ''' </summary>
    Private Sub toggleRaw()
        If (RigControl Is Nothing) OrElse NotOnFlex() Then
            Return
        End If
        ' First, start/stop the powerOn checking if raw is/isn't on.
        RigControl.Heartbeat = (RigControl.ReportRigOutput() = Radios.AllRadios.CommandReporting.raw)
        If RigControl.Heartbeat Then
            RigControl.ReportRigOutput(Radios.AllRadios.CommandReporting.none, "")
        Else
            RigControl.ReportRigOutput(Radios.AllRadios.CommandReporting.raw, "")
        End If
    End Sub
    ...
    Friend Sub ShowMenus()
        If (RigControl IsNot Nothing) AndAlso RigControl.IsOpen Then
            If (RigControl.RigFields IsNot Nothing) AndAlso _
    ...
    Friend Sub startPanning()
        ...
        If (RigControl IsNot Nothing) AndAlso RigControl.myCaps.HasCap(Radios.RigCaps.Caps.Pan) Then
            If OpenParms.RigDoesPanning Then
    ...
    Private Sub reackquireRtn()
        If OpenParms.ReackquireRig IsNot Nothing Then
            Try
                OpenParms.ReackquireRig()
            Catch ex As Exception
                Tracing.TraceLine("reackquire rig exception:" & ex.Message, TraceLevel.Error)
            End Try
        End If
    End Sub

    Private Sub clearRitRtn()
        Dim r As AllRadios.RITData
        If RigControl IsNot Nothing Then
            r = RigControl.RIT
            r.Value = 0
            RigControl.RIT = r
    ...
    Friend Sub toggle1()
        If (OpenParms IsNot Nothing) AndAlso (OpenParms.NextValue1 IsNot Nothing) AndAlso
           (RigControl IsNot Nothing) AndAlso RigControl.Power Then
    ...
    ' Region - remote audio
#Region "remote audio"
    Private Sub remoteAudioRtn()
        Try
            ' Note that remote audio is only supported for locally connected rigs.
            If (Not RigControl.RemoteRig) And
                    RigControl.myCaps.HasCap(RigCaps.Caps.RemoteAudio) Then
                remoteLan = Not remoteLan
    ...
    ''' <summary>
    ''' return a string for the Operations menu containing the action to perform
    ''' </summary>
    ''' <returns>String</returns>
    Private Function remoteMenuString() As String
        Dim str As String
        If remoteLan Then
            str = "off"
        Else
...
#Region "clusters"
    ...
    Private WithEvents clusterScreen As ClusterForm = Nothing
