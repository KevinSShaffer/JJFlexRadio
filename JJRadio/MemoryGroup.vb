...
Public Class MemoryGroup
    ...
    Friend allGroups As List(Of AllRadios.ScanGroup)
    Friend groupFile As Dictionary(Of String, AllRadios.ScanGroup)

    Private ReadOnly Property fileName As String
        Get
            Return BaseConfigDir & "\" & PersonalData.UniqueOpName(CurrentOp) & "_" & _
                CurrentRig.ModelName & "_groups.xml"
        End Get
    End Property

    Friend userGroups As List(Of AllRadios.ScanGroup)
    Friend ManageUserGroups As Boolean
    Friend SelectedGroup As AllRadios.ScanGroup
    ''' <summary>
    ''' The group list to display.
    '''  depends upon managing or scanning.
    ''' </summary>
    ''' <returns>list of ScanGroup</returns>
    Friend ReadOnly Property displayList As List(Of AllRadios.ScanGroup)
        Get
            Dim rv As List(Of AllRadios.ScanGroup) = allGroups
            If ManageUserGroups Then
                rv = userGroups
            End If
            Return rv
    ...
    Private Function fromFile() As Boolean
        ...
        Dim xs As New XmlSerializer(GetType(List(Of AllRadios.ExternalScanGroup)))
        Try
            Dim extGroups As List(Of AllRadios.ExternalScanGroup) = xs.Deserialize(fs)
            userGroups = New List(Of AllRadios.ScanGroup)
            For Each group As AllRadios.ExternalScanGroup In extgroups
                userGroups.Add(New AllRadios.ScanGroup(group, RigControl))
            Next
        Catch ex As Exception
            MsgBox(ex.Message)
            rv = False
    ...
    Private Function toFile() As Boolean
        ...
        Dim rv As Boolean = True
        Dim xs As New XmlSerializer(GetType(List(Of AllRadios.ExternalScanGroup)))
        Dim extGroups = New List(Of AllRadios.ExternalScanGroup)
        For Each group As AllRadios.ScanGroup In userGroups
            extGroups.Add(New AllRadios.ExternalScanGroup(group))
        Next
        Try
    ...
    Private Function compareNames(g1 As AllRadios.ScanGroup, g2 As AllRadios.ScanGroup)
    ...
    Private Sub setupLists()
        ...
        groupFile = New Dictionary(Of String, AllRadios.ScanGroup)
        For Each g As AllRadios.ScanGroup In allGroups
        ...
        For Each g As AllRadios.ScanGroup In displayList
    ...
    Friend Function Setup() As Boolean
        ...
        If (RigControl Is Nothing) OrElse (Not RigControl.MemoriesLoaded) OrElse (RigControl.NumberOfMemories = 0) Then
            MsgBox(mustBeLoaded)
            Return False
        End If
        allGroups = New List(Of AllRadios.ScanGroup)
        ' get groups from the rig
    ...
    Private Sub GroupsBox_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles GroupsCheckBox.SelectedIndexChanged, GroupsListBox.SelectedIndexChanged
        ...
        For Each mem As AllRadios.MemoryData In displayList(GroupsBox.SelectedIndex).Members
    ...
    Friend Sub AddUserGroup(group As AllRadios.ScanGroup)
    ...
    Friend Sub UpdateUserGroup(oldGroup As AllRadios.ScanGroup, group As AllRadios.ScanGroup)
        ...
        Dim g As AllRadios.ScanGroup = groupFile(oldGroup.Name)
        allGroups.Remove(g)
        userGroups.Remove(g)
    ...
    Friend Sub DeleteUserGroup(group As AllRadios.ScanGroup)
